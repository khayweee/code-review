"""Unit tests for sanitize-and-wrap intent handling (Milestone 3, issues #18 and #19).

Pure function tests -- `redact_secrets`, `strip_adversarial`, and `wrap_intent` have no
filesystem/subprocess/agent dependency, so nothing here is mocked. `IntentStep`'s tests
(issue #19) use a genuine hand-written `Agent`-protocol implementation as a spy, not a
mock library stand-in -- this repo's convention (see `tests/pipeline/test_executor.py`'s
docstring) is real `Step`/`Agent` implementations everywhere, including in tests.
"""
...
from code_review.steps.intent import (
    Intent,
    IntentStep,
    redact_secrets,
    strip_adversarial,
