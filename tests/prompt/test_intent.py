"""Unit tests for sanitize-and-wrap intent prompt construction (originally Milestone 3,
issues #18 and #19; moved here from `tests/steps/test_intent.py` in a later structural
refactor alongside the move of `wrap_intent`/`redact_secrets`/`strip_adversarial` from
`steps/intent.py` into `code_review.prompt.intent`).
...
from code_review.prompt.intent import redact_secrets, strip_adversarial, wrap_intent
