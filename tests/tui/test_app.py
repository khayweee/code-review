...
def test_review_app_keeps_a_finished_activity_under_its_starting_step_across_a_fast_handoff() -> (
    ...
    own consuming code (`_consume_activities`' old "tag with `self._running_step` at
    receipt time" logic -- see that method's current docstring for the full race).
    ...
    for `run_git`'s real subprocess spawn) lets the worker tag "started" while RebaseStep is
    still the running step, then -- with no further checkpoint -- RebaseStep's "completed"
    ...
    "ReviewStep". Before the fix, this either raised (`KeyError` in `backfill_activities`,
    exactly as seen against a real `RebaseStep` run) or silently split the activity's two
    halves across two different rows; now it renders once, correctly, under RebaseStep.
    """
    ...
    async def events() -> AsyncIterator[StepEvent]:
        ...
        async with relay.activity("git rebase origin/main"):
            ...
            # the activity worker its one chance to dequeue and tag "started" while
            # RebaseStep is still `self._running_step`.
            await asyncio.sleep(0)
