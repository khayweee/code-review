...
it in sync code. `StepOutcome.findings` is typed as bare `object`, not the Milestone 4
...
Once the fix/approval loop (Milestone 6) lands, record here: the fail-safe-default
regression test name(s) and the bounded-vs-unbounded fix-round asymmetry. Milestone 8
