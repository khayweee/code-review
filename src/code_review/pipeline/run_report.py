...
def _format_token_counts(input_tokens: int | None, output_tokens: int | None) -> str | None:
    if input_tokens is not None and output_tokens is not None:
        return f"{input_tokens:,} in / {output_tokens:,} out"
    if input_tokens is not None:
        return f"{input_tokens:,} in"
    if output_tokens is not None:
        return f"{output_tokens:,} out"
    return None


def _format_usage(
    input_tokens: int | None, output_tokens: int | None, total_cost_usd: float | None
) -> str:
    """One usage line, e.g. `"8,000 in / 4,000 out ($0.0800)"` -- omits whichever of
    tokens/cost is `None`. 4 decimal places on cost since this repo's real per-call costs are
    typically well under $1 (2 decimals would silently show "$0.00")."""

    tokens = _format_token_counts(input_tokens, output_tokens)
...
def format_run_report(
    ...
    totals = _format_usage(
        report.total_input_tokens, report.total_output_tokens, report.total_cost_usd
    )
    ...
    for step_usage in report.per_step:
        ...
        step_totals = _format_usage(usage.input_tokens, usage.output_tokens, usage.total_cost_usd)
