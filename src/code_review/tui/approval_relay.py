"""`ApprovalRelay`: queues a parked step's approve/skip/abort decision for a human to answer.
...
`future.set_result(decision)` to resolve the human's approve/skip/abort choice back to
...
subprocess; this collects a three-way `Decision` for a parked pipeline step. Mirrors
...
Decision = Literal["approve", "skip", "abort"]
...
class ApprovalRelay:
    ...
    def __init__(self) -> None:
        self._queue: asyncio.Queue[tuple[str, StepOutcome, asyncio.Future[Decision]]] = (
            asyncio.Queue()
        )

    async def request_approval(self, step_name: str, outcome: StepOutcome) -> Decision:
        ...
        once a step's `StepOutcome.needs_approval` is True. Blocks until whatever is
        consuming `next_request` resolves the future it was handed for this call.
        """
    ...
    async def next_request(self) -> tuple[str, StepOutcome, asyncio.Future[Decision]]:
        """Consumed by `ReviewApp`'s worker: the next queued park request -- the parked
        step's name, its `StepOutcome`, and the future to resolve with a `Decision`.

        Blocks until a `request_approval` call queues one. The caller is expected to
        collect a decision from the human and call `future.set_result(decision)` to
        unblock the matching `request_approval` call.
        """
