"""Sanitize-and-wrap prompt construction for intent text -- originally Milestone 3 (see
docs/ROADMAP.md), moved here from `steps/intent.py` in a later structural refactor to
separate prompt-construction logic from step-orchestration logic (`steps/` now depends on
this package rather than mixing the two concerns in one module).

This module is the one sanitize-and-wrap function (redact secrets, strip adversarial
delimiter shapes, wrap in a clearly-marked block), meant to be reused at every prompt site
that embeds intent text. It operates on plain strings only -- nothing here needs the
`Intent` dataclass type (`steps/intent.py`), which is why it lives in this leaf package
